 ...
 * neighbors:
 *
 * // Where chf is an instance of a CompactHeightfield. const float cs = chf.cs;
 * const float ch = chf.ch; for (int y = 0; y < chf.height; ++y) { for (int x =
 * 0; x < chf.width; ++x) { // Deriving the minimum corner of the grid location.
 * const float fx = chf.bmin[0] + x*cs; const float fz = chf.bmin[2] + y*cs; //
 * Get the cell for the grid location then iterate // up the column. const
 * CompactCell& c = chf.cells[x+y*chf.width]; for (unsigned i = c.index, ni =
 * c.index+c.count; i < ni; ++i) { const CompactSpan& s = chf.spans[i]; Deriving
 * the minimum (floor) of the span. const float fy = chf.bmin[1] + (s.y+1)*ch;
 * // Testing the area assignment of the span. if (chf.areas[i] ==
